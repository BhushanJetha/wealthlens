...
export default async function FinancialHealthPage() {
  ...
  const [
    ...
    goldRes, bondRes, etfRes, loansRes, txnsRes, goalsRes, giRes,
  ] = await Promise.all([
    ...
    supabase.from('goals').select('id,name,target_amount,currency').eq('user_id', user!.id).eq('status', 'active'),
    supabase.from('goal_investments').select('investment_type,investment_id').eq('user_id', user!.id),
  ])
